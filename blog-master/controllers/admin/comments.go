...
func (this *CommentsController) Add() {
	if this.Ctx.Request.Method == "POST" {
		...
		replypk, _ := strconv.Atoi(strings.TrimSpace(this.GetString("reply_pk")))
		...
		fmt.Println(reflect.TypeOf(comment_content))
		...
		if comment_content != "" && security_hash != "" {
			checkstr := models.Md5([]byte(comment_content + timestamp + "@YO!r52w!D2*I%Ov"))
			if checkstr == security_hash {
				...
				comment.Reply_pk = int64(replypk)
